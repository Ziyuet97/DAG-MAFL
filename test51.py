...
def main():
    ...
    # 初始化所有客户端的模型
    client_models = [FastMNIST().to(DEVICE) for _ in range(NUM_CLIENTS)]
    ...
    for round_idx in range(1, NUM_ROUNDS + 1):
        ...
        for client_idx in selected_clients:
            ...
            for epoch in range(NUM_EPOCHS):
                train(client_models[client_idx], train_loaders[client_idx], optimizer, DEVICE)
        
        # 计算所有10个节点的最新模型的平均准确度
        total_acc = 0.0
        for node_id in range(NUM_CLIENTS):
            _, acc = test(client_models[node_id], client_test_loaders[node_id], DEVICE)
            total_acc += acc
        
        avg_acc = total_acc / NUM_CLIENTS
        local_avg_accuracies.append(avg_acc)
        print(f"Round {round_idx} - Average Local Accuracy (all nodes): {avg_acc:.2f}%")
        
        # 准备用于聚合的模型和数据大小
        selected_models = [client_models[i] for i in selected_clients]
        selected_data_sizes = [client_data_sizes[i] for i in selected_clients]
        
        # 使用FedAvg聚合模型参数
        global_model = federated_avg(selected_models, selected_data_sizes)
    ...
    # 绘制本地平均准确度
    plt.plot(range(1, NUM_ROUNDS + 1), local_avg_accuracies, label='Average Local Accuracy (all nodes)', marker='x')
