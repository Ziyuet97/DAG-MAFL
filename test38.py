...
GLOBAL_AGGREGATION_INTERVAL = 10  # 每10个交易执行一次全局聚合
...
class DAGBlockchainFL:
    def __init__(self, num_nodes=10, node_primary_digits=None):
        ...
        self.node_positions = {}
        self.current_id = 0
        self.tx_params = []  # 存储交易参数的列表
        self.tx_accuracies = []  # 存储每个交易的精度（针对主要数字）
        self.tx_global_accuracies = []  # 存储每个交易的全局精度（所有数字）
        self.tx_importance_scores = []  # 存储每个交易的参数重要性分数
        self.node_primary_digits = node_primary_digits  # 每个节点主要关注的数字
        self.round = 0
        self._init_genesis()
        self.last_global_tx = 0  # 记录上一次全局节点交易的ID
    ...
    def _init_genesis(self):
        ...
        self.tx_params.append(initial_model.state_dict())  # 初始模型参数
        self.tx_accuracies.append(0.0)  # 初始精度为0
        self.tx_global_accuracies.append(0.0)  # 初始全局精度为0
        self.tx_importance_scores.append(0.0)  # 初始重要性分数为0
        self.current_id += 1
    ...
    def evaluate_model_accuracy(self, model_params, node_id=None, use_global=False):
        ...
        # 如果指定了使用全局测试集或没有指定节点ID，则使用全局测试集
        if use_global or node_id is None:
            return evaluate(model, test_loader)
        # 否则使用节点专用测试集（只包含其主要数字）
    ...
    def calculate_local_average_accuracy(self, current_round):
        """计算当前轮次所有活跃子节点的平均本地精度"""
        if current_round not in self.round_transactions or not self.round_transactions[current_round]:
            return 0.0
            
        total_acc = 0.0
        total_txs = len(self.round_transactions[current_round])
        
        for tx_id, node_id, acc in self.round_transactions[current_round]:
            total_acc += acc
            
        avg_acc = total_acc / total_txs if total_txs > 0 else 0
        print(f"\n📊 Round {current_round} local average accuracy: {avg_acc:.2f}%")
        return avg_acc
    ...
    def run_simulation(self, rounds=NUM_ROUNDS):
        ...
        for r in range(rounds):
            ...
            # 计算当前轮次的本地平均精度
            local_avg_acc = self.calculate_local_average_accuracy(current_round)
