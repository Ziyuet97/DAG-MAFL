...
for round in range(NUM_ROUNDS):
    ...
    for client in clients:
        # 测试当前客户端的模型（无论是否参与本轮训练）
        _, acc = client.test()
        local_accuracy_history[client.client_id].append(acc)
        total_acc += acc
    avg_local_acc = total_acc / NUM_CLIENTS
    avg_local_accuracy_history.append(avg_local_acc)
    print(f"Round {round}, Avg Local Acc: {avg_local_acc:.4f}")

# 绘制结果（包含全局和平均本地准确率）
plt.figure(figsize=(10, 6))
plt.plot(global_accuracy_history, label='Global Model')
plt.plot(avg_local_accuracy_history, label='Average Local Models')
...
plt.show()
print("local")
print(avg_local_accuracy_history)
print("global")
