...
average_local_accuracy_history = []  # To store average local accuracy per round
...
for round in range(NUM_ROUNDS):
    ...
    for client in clients:
        # Use the client's current model (which has been trained this round if selected)
        acc = evaluate(client.model, client.test_loader)
        total_acc += acc
        local_accuracy_history[client.client_id].append(acc)
    avg_local_acc = total_acc / NUM_CLIENTS
    average_local_accuracy_history.append(avg_local_acc)
    ...
    if transaction_count % GLOBAL_AGGREGATION_INTERVAL == 0:
        ...
        print(f"Round {round}, Global Acc: {global_acc:.4f}, Avg Local Acc: {avg_local_acc:.4f}")

# 可视化结果 (modified to include average local accuracy)
plt.figure(figsize=(10, 6))
plt.plot(global_accuracy_history, label='Global Model')
plt.plot(average_local_accuracy_history, label='Average Local Accuracy', linestyle='--', linewidth=2)
for client_id, accs in local_accuracy_history.items():
    plt.plot(accs, label=f'Client {client_id}', alpha=0.3)
...
print("Local accuracy history:", local_accuracy_history)
print("Global accuracy history:", global_accuracy_history)
print("Average local accuracy history:", average_local_accuracy_history)