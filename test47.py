...
class FastMNIST(nn.Module):
    ...
    def forward(self, x):
        x = x.view(-1, 784)  # 展平MNIST图像 [batch_size, 1, 28, 28] -> [batch_size, 784]
        x = F.relu(self.fc1(x))
        return self.fc2(x)
...
def client_update(model, loader, optimizer):
    ...
    for _ in range(NUM_EPOCHS):
        for X, y in loader:
            ...
            optimizer.zero_grad()
            output = model(X.view(-1, 784))  # 添加展平操作适配MNIST
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for X, y in test_loader:
            X, y = X.to(DEVICE), y.to(DEVICE)
            output = model(X.view(-1, 784))  # 添加展平操作
...
class DAGBlockchainFL:
    ...
    def _init_genesis(self):
        initial_model = FastMNIST()  # 改为MNIST模型
...
global_model = FastMNIST().to(DEVICE)  # 初始化全局模型
